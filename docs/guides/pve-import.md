...
| ベースイメージ | Ubuntu 26.04 Resolute (cloud image) |
...
4. 完了後、サマリーページの **Artifacts** から
   `devtool-resolute-amd64-qcow2` をダウンロード
5. zip を展開して以下を取り出す:
   ```
   devtool-resolute-amd64.qcow2
   devtool-resolute-amd64.qcow2.sha256
   ```
6. チェックサム確認:
   ```bash
   sha256sum --check devtool-resolute-amd64.qcow2.sha256
   ```
...
```bash
VERSION="v0.1.0"
curl --location --output devtool-resolute-amd64.qcow2 \
  "https://github.com/naa0yama/devtool-wsl2/releases/download/${VERSION}/devtool-resolute-amd64.qcow2"
curl --location --output devtool-resolute-amd64.qcow2.sha256 \
  "https://github.com/naa0yama/devtool-wsl2/releases/download/${VERSION}/devtool-resolute-amd64.qcow2.sha256"
sha256sum --check devtool-resolute-amd64.qcow2.sha256
```
...
scp devtool-resolute-amd64.qcow2 \
...
```bash
ls -lh /var/lib/vz/images/devtool-resolute-amd64.qcow2
```
...
```bash
VMID=9000
TEMPLATE_NAME="devtool-resolute-amd64"
STORAGE="local-lvm"
IMAGE="/var/lib/vz/images/devtool-resolute-amd64.qcow2"
```
