...
# File
if [ ! -f "${HOME}/.gitconfig" ] && [ -f "${USERPROFILE}/.gitconfig" ]; then
	log_info "Copy .gitconfig from Windows"
	cp -v "${USERPROFILE}/.gitconfig" "${HOME}"
	chmod 0644 "${HOME}/.gitconfig"
fi
if [ ! -f "${HOME}/.gitignore_global" ] && [ -f "${USERPROFILE}/.gitignore_global" ]; then
...
# Directory
if [ ! -d "${HOME}/.gitconfig.d" ] && [ -d "${USERPROFILE}/.gitconfig.d" ]; then
