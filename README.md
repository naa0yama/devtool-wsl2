...
以前は、 WSL2 に都度コマンドを打って環境構築していたが似たような環境が複数必要になるため自動化し環境構築にかかる時間を省力化した。現在は WSL2 rootfs、既存 Ubuntu ホスト (Proxmox VE の VM を含む) への bootstrap の 2 経路をサポートする。

## 対応環境と経路の使い分け

Single Source of Truth (SSOT) として `scripts/provision/{system,user}/*.sh` を保持し、2 種類の配布形態から呼び出す:

- **A. WSL2 rootfs** — Windows Terminal + PowerShell 経由で自動 Import。個人開発機の標準経路。詳細は下の「使い方」節。
- **B. bootstrap.sh (既存 Ubuntu / VM / container)** — 既存 Ubuntu 24.04 host に `curl | bash` で SSOT スクリプトを流し込む。CI・Proxmox VE の VM・ゲスト VM で使う。エントリ: `scripts/provision/bootstrap.sh`。Proxmox VE では stock Ubuntu cloud image を import し、この oneliner を実行する。手順は [docs/guides/pve-import.md](docs/guides/pve-import.md) を参照。

`setup.sh` は `DEVTOOL_SETUP_MODE` (auto: `wsl2` / `vm` / `remote`) を検出し、WSL2 では Windows OpenSSH/GPG bridge を、VM (bootstrap.sh 経由で構築した Proxmox VE 等) では docker-cleanup / cargo-sweep タイマーのみを、SSH のみの remote host では GPG/sshd 設定のみを導入する。
